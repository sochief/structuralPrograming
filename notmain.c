...
int main()
{
    ...
    while (exit != 1)
    {
        ...
        case 6:
            ...
            printf("The %d_ClassList.txt file is ready\n", new_id);
            break;
        case 7:
...
void dumpClass(ClassList *classList)
{
    ...
    while (current)
    {
        fprintf(fptr, "%d/%d/%d/%d/%s\n", current->id, current->credits, current->capacity, current->profId, current->name);
        current = current->next;
    }
    fclose(fptr);
}
void dumpClassList(ClassList *classList, RelationList *relationList, StudList *studList)
{
    ...
    while (current)
    {
        itoa(current->id, char_id, 10);
        listClassStuds(char_id, relationList, studList);
        current = current->next;
    }
}