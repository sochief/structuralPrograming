...
int main()
{
    ...
    while (exit != 1)
    {
        ...
        case 4:
            ...
            {
                printf("currentRelation is NULL");
                ...
                printSingleRelation(relationList->head);
            }
            else
            {
                updateRelationStatus(currentRelation->id, relationList);
            }
            break;
        case 5:
...
void addRelation(RelationList *relationList, char *char_studentId, char *char_classId)
{
    ...
    char date = "01/13/2020";
    strcpy(new_relation->registDate, date);
...
void listStudClasses(char *id, RelationList *relationList, ClassList *classList)
{
    ...
    while (current)
    {
        if (studId == current->studentId)
        {
            ...
            printf("Current Status: %s", current->status);
        }
        current = current->next;
    }
}
...
{
    FILE *fp;
    char fileName = strcat(id, "_classList.txt");
    fp = fopen(fileName, "w");
    if (fp == NULL)
    {
